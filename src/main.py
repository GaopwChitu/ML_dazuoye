...
def parse_global_args(parser):
    ...
    parser.add_argument('--load', type=int, default=0,
                        help='Whether load models and continue to train')
    parser.add_argument('--train', type=int, default=1,
                        help='To train the models or not.')
...
def save_rec_results(dataset, runner, topk):
    ...
    if init_args.model_mode == 'CTR':  # CTR task
        ...
        rec_df.to_csv(result_path, sep=args.sep, index=False)
    elif init_args.model_mode in ['Impression', 'General',
                                  'Sequential']:  # List-wise reranking task: Impression is reranking task for general/seq baseranker. General/Sequential is reranking task for rerankers with general/sequential input.
...
if __name__ == '__main__':
    ...
    reader_name = eval('{0}.{0}'.format(model_name.reader))  # models chooses the reader
    runner_name = eval('{0}.{0}'.format(model_name.runner))  # models chooses the runner
    ...
    if args.model_path == '':
        args.model_path = '../models/{}/{}.pt'.format(init_args.model_name + init_args.model_mode, log_file_name)
