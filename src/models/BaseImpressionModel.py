...
class ImpressionModel(GeneralModel):
    ...
    def loss(self, out_dict: dict, target=None):
        ...
        mask = torch.where(target == -1, target,
                           torch.zeros_like(target)) + 1  # only non pad item is 1,shape like prediction
        ...
        if 'BPR' in self.loss_n:  # Do not need to consider test_have_neg because the score_diff_mask has masked the places with no neg samples
            valid_mask = mask.unsqueeze(dim=-1) * mask.unsqueeze(dim=-1).transpose(-1,
                                                                                   -2)  # valid_mask[][i][j] is only 1 when mask[][i], mask[][j] are all 1
            pos_mask = (torch.arange(cand_len).unsqueeze(0).repeat(batch_size, 1) < self.train_max_pos_item).to(
                self.device)  # pos positions
            neg_mask = (torch.arange(cand_len).unsqueeze(0).repeat(batch_size, 1) >= self.train_max_pos_item).to(
                self.device)  # neg positions
            select_mask = pos_mask.unsqueeze(dim=-1) * neg_mask.unsqueeze(dim=-1).transpose(-1,
                                                                                            -2) * valid_mask  # get all valid mask in the two-dimensional matrix, select_mask[][i][j] is only 1 when mask[][i] is positive 1 (i<pos_num), mask[][j] is negative 1 (j>=pos_num)
            score_diff = prediction.unsqueeze(dim=-1) - prediction.unsqueeze(dim=-1).transpose(-1,
                                                                                               -2)  # batch * impression list * impression list
            ...
            neg_pred = torch.where(neg_mask * mask == 1, prediction,
                                   -torch.tensor(float("Inf")).float().to(self.device))
            neg_softmax = (neg_pred - neg_pred.max()).softmax(
                dim=1)  # [batch_size, max_length_of_candidate_list], only valid negative sample has softmax weights
            if 'hard' in self.loss_n:  # Higher weights for lower-score positive items
                pos_pred = torch.where(pos_mask * mask == 1, prediction,
                                       torch.tensor(float("Inf")).float().to(self.device))
                pos_softmax = (pos_pred.min() - pos_pred).softmax(
                    dim=1)  # [batch_size, max_length_of_candidate_list], only valid positive sample has softmax weights
            else:  # Higher weights for high-score positive items
                pos_pred = torch.where(pos_mask * mask == 1, prediction,
                                       -torch.tensor(float("Inf")).float().to(self.device))
                pos_softmax = (pos_pred - pos_pred.max()).softmax(
                    dim=1)  # [batch_size, max_length_of_candidate_list], only valid positive sample has softmax weights
            ...
            if 'after' in self.loss_n:  # Do reweight across pos and neg samples after log-sigmoid. If only 1 pos and 1 neg, degrade to the BPR loss. The BPR loss is -log(softmax(x)). Softplus is log(1+e^x), which is equivalent of -log(softmax(-x)).
                loss = ((F.softplus(-score_diff_mask) * neg_softmax.unsqueeze(dim=1)).sum(dim=-1) * pos_softmax).sum(
                    dim=-1)
                loss = loss.mean()
                ...
                score_diff_mask = torch.where(select_mask == 1, score_diff_mask,
                                              -torch.tensor(float("Inf")).float().to(self.device))
                loss = -((score_diff_mask.sigmoid() * neg_softmax.unsqueeze(dim=1)).sum(dim=-1) * pos_softmax).sum(
                    dim=-1).log()
                loss = loss.mean()
            return loss
            ...
            prediction_softmax = torch.where(mask == 1, prediction_softmax, torch.ones_like(
                prediction_softmax))  # make sure that paddings are 1, so that after log they are 0
            ...
            pre_softmax = (prediction - prediction.max(dim=1, keepdim=True)[0]).softmax(
                dim=1)  # [batch_size, max_length_of_candidate_list]
            ...
            prediction_softmax1 = torch.where(mask == 1, prediction_softmax, torch.ones_like(
                prediction_softmax))  # make sure that paddings are 1, so that after log they are 0
            loss_1 = -(target_softmax * prediction_softmax1.log()).sum(dim=1)

            prediction_softmax2 = torch.where(mask == 1, prediction_softmax, torch.zeros_like(
                prediction_softmax))  # make sure that paddings are 0, so that after log 1-they are 0
            prediction_softmax2 = torch.where(prediction_softmax2 != 1, prediction_softmax2, torch.zeros_like(
                prediction_softmax2))  # make sure that no 1 in prediction_softmax2, because it only happens when only 1 sample, so its loss must be 0
    ...
    class Dataset(GeneralModel.Dataset):
        ...
        def collate_batch(self, feed_dicts: List[dict]):
            ...
            if pos_items.shape[-1] < self.pos_len:  # padding positive items
                pos_items = torch.cat((pos_items, torch.zeros(pos_items.shape[0], self.pos_len - pos_items.shape[-1])),
                                      dim=-1)
            neg_items = feed_dict['neg_items']
            if neg_items.shape[-1] < self.neg_len:  # padding negative items
                neg_items = torch.cat((neg_items, torch.zeros(neg_items.shape[0], self.neg_len - neg_items.shape[-1])),
                                      dim=-1)
        ...
        def actions_before_epoch(self):
            ...
            # Or else the negative sampling function of general models dataset will be called.
            ...
            for i, u in enumerate(self.data['user_id']):
                ...
                for j in range(self.models.num_neg):
