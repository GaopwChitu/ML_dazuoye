...
class BaseModel(nn.Module):
    reader, runner = None, None  # choose helpers in specific models classes
    extra_log_args = []
    ...
    # logging.info('Save models to ' + model_path[:50] + '...')

    def load_model(self, model_path=None):
        if model_path is None:
            model_path = self.model_path
        self.load_state_dict(torch.load(model_path))
        logging.info('Load models from ' + model_path)
    ...
    class Dataset(BaseDataset):
        def __init__(self, model, corpus, phase: str):
            self.model = model  # models object reference
        ...
        def collate_batch(self, feed_dicts: List[dict]) -> dict:
            ...
            for key in feed_dicts[0]:
                ...
                # -------- tensorize --------
                if stack_val.dtype == object:  # variable-length sequence (e.g., history)
                    tensor_list = []
                    for x in stack_val:
                        t = torch.from_numpy(x)
                        # index / sequence must be long
                        if t.dtype in (torch.int32, torch.int64):
                            t = t.long()
                        tensor_list.append(t)
                    feed_dict[key] = pad_sequence(tensor_list, batch_first=True)

                else:
                    t = torch.from_numpy(stack_val)
                    # index / id must be long (GPU embedding requirement)
                    if t.dtype in (torch.int32, torch.int64):
                        t = t.long()
                    feed_dict[key] = t
...
class GeneralModel(BaseModel):
    ...
    def loss(self, out_dict: dict) -> torch.Tensor:
        ...
        loss = -(((pos_pred[:, None] - neg_pred).sigmoid() * neg_softmax).sum(dim=1)).clamp(min=1e-8,
                                                                                            max=1 - 1e-8).log().mean()
...
class CTRModel(GeneralModel):
    ...
    def loss(self, out_dict: dict) -> torch.Tensor:
        """
        MSE/BCE loss for CTR models, out_dict should include 'label' and 'prediction' as keys
        """
        if self.loss_n == 'BCE':
